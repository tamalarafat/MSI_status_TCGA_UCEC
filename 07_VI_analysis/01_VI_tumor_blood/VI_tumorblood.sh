...
WORKROOT=/pfssmain/departments/pathologie/users/tamyas01/work_dir/sarek/vi_analysis/vi_tumorblood
...
nextflow run "$PIPE" \
  ...
  -c "$CFG" \
  -params-file "$PARAMS" \
  --input /pfssmain/departments/pathologie/users/tamyas01/work_dir/sarek/vi_config/vi_samplesheet_tumor_blood.csv \
  --outdir "$RESULTSDIR" \
